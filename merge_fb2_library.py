...
- [ ] Поддержка других кодировок FB2 (например, cp1251) — сейчас используется utf-8 с ignore
- [ ] Добавление хэша книги (SHA-256) для более точного обнаружения дубликатов при совпадении метаданных
- [ ] Опция "тестовый запуск" — без записи на диск, только статистика
- [ ] Автоматическое резервное копирование library.db перед изменениями
- [ ] Поддержка подпапок в выходной директории (разделение по годам/месяцам)
- [ ] Возможность продолжить с последнего .tmp.zip после сбоя (восстановление состояния)
- [ ] Ведение истории мёрджей (журнал в meta: дата, src1, src2, результат)
- [ ] Проверка целостности ZIP-архивов перед чтением (is_zipfile)
- [ ] Многопоточная обработка архивов (если CPU позволяет)
- [ ] GUI: темная тема, масштабируемость окна, сохранение путей между запусками
- [ ] Экспорт отчёта в HTML/PDF (с графиками: динамика добавления книг и т.д.)
- [ ] Уведомления (звук, системный toast) по завершению
'''
