...
def fast_parse(data):
    ...
    author = ""
    title = ""
    a = re.search(r"<last-name>(.*?)</last-name>", text, re.DOTALL)
    if a:
        author = a.group(1).strip()
    t = re.search(r"<book-title>(.*?)</book-title>", text, re.DOTALL)
    if t:
...
def worker(src1, src2, out_dir, ui_q):
    ...
    # Переименовываем временные архивы
    if not STOP_FLAG:
        for f in os.listdir(out_dir):
            if f.endswith(".tmp.zip"):
                ...
                # ❌ УБРАТЬ: больше не добавляем в indexed_archives при создании
                # archive_name = os.path.basename(final)
                # cur.execute("INSERT OR IGNORE INTO indexed_archives (archive_name) VALUES (?)", (archive_name,))
        log("✅ Все архивы переименованы.")
    else:
    ...
    if not STOP_FLAG:
        ...
        log("⚠️ Работа прервана — метка 'full_index_complete' НЕ установлена.")
